const TECH_KEYWORDS = [
  "javascript", "python", "java", "c++", "react", "node",
  "express", "mongodb", "mysql", "postgres", "html", "css",
  "git", "github", "docker", "aws", "api", "rest",
  "authentication", "jwt", "sql", "linux"
];

const ACTION_VERBS = [
  "built", "developed", "implemented", "designed", "created",
  "optimized", "led", "managed", "improved", "deployed",
  "integrated", "automated", "analyzed"
];

const SECTIONS = [
  "education", "skills", "projects", "experience",
  "internship", "certifications", "achievements"
];
...
export const calculateATSScore = (resumeText) => {
  ...
  breakdown.experience = Math.min(
    15,
    expIndicators.filter(e => text.includes(e)).length * 3
  );
  score += breakdown.experience;

  // 4️⃣ Action verbs (10)
  breakdown.actionVerbs = Math.min(
    10,
    ACTION_VERBS.filter(v => text.includes(v)).length * 2
  );
