...
`;
    ...
    const response = await AI.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a helpful blog writer. Try to finish sentences completely.',
        },
        {
          role: 'user',
          content: aiPrompt,
    ...
    titles = titles.map((t) => t.trim()).filter((t) => t.length > 30);
    ...
    const content = titles.map((t, i) => `${i + 1}. ${t.endsWith('.') ? t : t + '.'}`).join('\n');
...
export const resumeReview = async (req, res) => {
  try {
    ...
    const resumeText = resumeTextRaw.replace(/\r/g, '').trim().slice(0, 3000); // ATS + Groq safe
...
`;
    ...
    if (!content || content.length < 400) {
      finalContent =
        'Resume analysis could not be generated at this time. ATS score is shown below.';
    }
