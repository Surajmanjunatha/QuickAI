  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setAtsBreakdown(data.atsBreakdown);
      } else {
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      <div className="w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-150 ">
        <div className="relative flex items-center gap-3">
          ...
          {atsScore !== null && (
            <div
              className="absolute right-0 top-0 flex items-center justify-center w-11 h-11 rounded-full border-4 bg-white"
              style={{
                borderColor: atsScore >= 75 ? '#22c55e' : atsScore >= 50 ? '#eab308' : '#ef4444',
              }}
            >
              <span className="text-xs font-bold text-slate-700">{atsScore}%</span>
            </div>
          )}
        </div>
        {!content ? (
